#include <Cdd/CddExtFlash/CddExtFlash.h>


// Function to enable writing
void CddExtFlashWriteEnable(void)
{
  // Send Write Enable command
  CddSpiTransfer(CMD_WRITE_ENABLE);
}

// Function to wait until the chip is not busy
void CddExtFlashWaitForRead(void)
{
  while (CddSpiTransfer(0x05U) & 0x01U); // Poll Status Register until busy bit is cleared
}

// Function to write data to a sector
void CddExtFlashWriteSector(uint32_t sector_address, uint8_t *data, uint32_t len)
{
  // Enable writing
  CddExtFlashWriteEnable();

  // Send Page Program command
  CddSpiTransfer(CMD_PAGE_PROGRAM);

  CddSpiTransfer((sector_address >> 16) & 0xFF);

  // Address byte 2
  CddSpiTransfer((sector_address >> 8) & 0xFF);

  // Address byte 1
  CddSpiTransfer(sector_address & 0xFF);

  // Address byte 0
  // Write data
  for (uint32_t i = 0; i < len; i++)
  {
    CddSpiTransfer(data[i]);
  }

  // Wait for completion
  CddExtFlashWaitForRead();
}


